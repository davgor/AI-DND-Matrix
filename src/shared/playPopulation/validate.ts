...
function optionalPlaceField<T>(
  value: T | undefined,
  key: keyof PlaceProposal
): Partial<PlaceProposal> {
  return value !== undefined ? ({ [key]: value } as Partial<PlaceProposal>) : {}
}

function parseValidatedPlaceProposal(body: Record<string, unknown>): PlaceProposal | undefined {
  const key = readNonEmptyString(body['key'])
  const name = readNonEmptyString(body['name'])
  ...
  const parentRegionId = readNonEmptyString(body['parentRegionId'] ?? body['parent_region_id'])
  const parentRegionKey = readNonEmptyString(body['parentRegionKey'] ?? body['parent_region_key'])
  return {
    key,
    name,
    description,
    ...optionalPlaceField(parentRegionId, 'parentRegionId'),
    ...optionalPlaceField(parentRegionKey, 'parentRegionKey')
  }
}

export function isValidPlaceProposal(value: unknown): value is PlaceProposal {
  if (!value || typeof value !== 'object') {
    return false
  }
  return parseValidatedPlaceProposal(value as Record<string, unknown>) !== undefined
}

export function parsePlaceProposal(value: unknown): PlaceProposal | undefined {
  if (!value || typeof value !== 'object') {
    return undefined
  }
  return parseValidatedPlaceProposal(value as Record<string, unknown>)
}
