...
import {
  createCompanionFaceTokenSchedulerDeps,
...
type ResolveAcceptPlayerResult =
  | { ok: true; character: ReturnType<typeof assertCompanionsPlayer> }
  | { ok: false; reason: 'not_found' | 'invalid_phase' }

function resolveAcceptPlayer(
  db: Database.Database,
  input: CompanionsAcceptInput
): ResolveAcceptPlayerResult {
  try {
    return { ok: true, character: assertCompanionsPlayer(db, input) }
  } catch (error) {
...
export async function acceptCompanionPreview(
  ...
  provider: Provider,
  input: CompanionsAcceptInput,
  faceTokenSchedulerDeps?: CompanionFaceTokenSchedulerDeps
): Promise<CompanionsAcceptResult> {
  const resolved = resolveAcceptPlayer(db, input)
  if (!resolved.ok) {
    return resolved
  }
  ...
  maybeEnqueueCompanionFaceTokenAfterAccept(
    faceTokenSchedulerDeps ?? createCompanionFaceTokenSchedulerDeps(db),
    input.campaignId,
    member.id
  )
  return { ok: true }
