...
describe('getNpcCombatStats', () => {
  it('returns fixed villager constants', () => {
  ...
  it('maps each retired adventurer profile to a distinct stronger block', () => {
    ...
    expect(brawler.hp).toBeGreaterThan(VILLAGER_STATS.hp)
    expect(skirmisher.ac).toBeGreaterThan(VILLAGER_STATS.ac)
    expect(veteran.hp).toBeLessThanOrEqual(RETIRED_ADVENTURER_MAX_HP)
    expect(veteran.ac).toBeLessThanOrEqual(RETIRED_ADVENTURER_MAX_AC)
    expect(brawler).not.toEqual(skirmisher)
    expect(skirmisher).not.toEqual(veteran)
  })
