...
- [ ] `fillSkeleton(skeleton, rawLlmText)` (names flexible) fills all placeholders from labeled blocks
- [ ] Unit tests: happy path, missing token, extra/unknown token, malformed/unclosed tags, nested/prose noise around tags
- [ ] Placeholder substitution does not re-introduce unescaped quotes/control chars that break `JSON.parse` (escape policy documented in code/tests)
- [ ] No campaign stage prompts changed yet
